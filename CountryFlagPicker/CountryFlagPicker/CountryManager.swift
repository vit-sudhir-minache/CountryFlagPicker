...
public extension CountryManager {
    ...
    func fetchCountries(fromURLPath path: URL) throws -> [Country] {
        guard let rawData = try? Data(contentsOf: path),
              let countryCodes = try? PropertyListSerialization.propertyList(from: rawData, format: nil) as! [AnyObject] else {
            throw "[CountryManager] ❌ Missing countries plist file from path: \(path)"
        }
        
        // Sort country list by `countryName`
        let sortedCountries = countryCodes.map { Country(countryCode: $0 as! String) }
        let sort = sortedCountries.sorted { $0.countryName < $1.countryName }
        
        #if DEBUG
        print("[CountryManager] ✅ Succefully prepared list of \(sort.count) countries")
        #endif
